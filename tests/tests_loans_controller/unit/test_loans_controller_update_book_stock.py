...
def test_update_book_stock(mock_loans_controller_with_book_model):
    ...
    book_id = 1
    change = 1
    result = loans_controller.update_book_stock(book_id, change)

    mock_book_model.update_stock_by_id.assert_called_with(book_id, change)
...
def test_update_book_stock_failure(mock_loans_controller_with_book_model):
    ...
    with pytest.raises(ValueError) as e:
        loans_controller.update_book_stock(book_id, change)
