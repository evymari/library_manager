def test_send_due_today_email_success(notification_service_with_mocks):
    """
    Scenario: Successfully send due today email
      Given a valid user ID, book ID, and due date
      When the send_due_today_email function is called
    ...
    message = f"Dear user, the book 'The Great Gatsby' is due today ({due_date}). Please return it today."

    # When
    notification_service.send_due_today_email(user_id, book_id, due_date)
...
def test_send_due_today_email_user_email_error(notification_service_with_mocks):
    """
    ...
    # When
    result = notification_service.send_due_today_email(user_id, book_id, due_date)
...
def test_send_due_today_email_book_title_error(notification_service_with_mocks):
    """
